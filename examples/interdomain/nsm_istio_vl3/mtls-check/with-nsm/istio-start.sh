...
exec sudo -E -u ${EXEC_USER} -s /bin/bash -c "ulimit -n ${currentLimit}; INSTANCE_IP=${ISTIO_SVC_IP} POD_NAME=${POD_NAME} POD_NAMESPACE=${NS} /usr/local/bin/dlv --continue --accept-multiclient --listen=:40000 --check-go-version=false --headless=true --api-version=2 --log=true --log-output=debugger,debuglineerr,gdbwire,lldbout,rpc exec ${ISTIO_BIN_BASE}/pilot-agent -- proxy ${ISTIO_AGENT_FLAGS_ARRAY[*]} 2>> ${ISTIO_LOG_DIR}/istio.err.log >> ${ISTIO_LOG_DIR}/istio.log"
fi
